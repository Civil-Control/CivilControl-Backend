...
import PSG.backEnd.service.implementation.InsurancePolicyPaymentService;
...
public class InsurancePolicyController {
    ...
    private final InsurancePolicyPaymentService insurancePolicyPaymentService;
    ...
    @Operation(summary = "Register a payment for an insurance policy",
            description = "Creates a new payment record linked to the unified PaymentDetails system. Supports CASH, TRANSFER, and CHECK methods.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Payment successfully registered"),
            @ApiResponse(responseCode = "400", description = "Invalid input data or validation error"),
            @ApiResponse(responseCode = "404", description = "Insurance policy not found")
    })
    public ResponseEntity<InsurancePolicyPaymentResponseDTO> createPayment(
            @Parameter(description = "Insurance policy unique identifier", required = true) @PathVariable Long insurancePolicyId,
            @Validated(OnCreate.class) @RequestBody InsurancePolicyPaymentCreateDTO dto) {
        InsurancePolicyPaymentResponseDTO created = insurancePolicyPaymentService.createPayment(insurancePolicyId, dto);
    ...
    public ResponseEntity<List<InsurancePolicyPaymentResponseDTO>> getPaymentsByPolicyId(
            @Parameter(description = "Insurance policy unique identifier", required = true) @PathVariable Long insurancePolicyId) {
        List<InsurancePolicyPaymentResponseDTO> payments = insurancePolicyPaymentService.getPaymentsByPolicyId(insurancePolicyId);
    ...
    @Operation(summary = "Get payment by ID",
            description = "Retrieves a specific policy payment by its unique identifier.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Payment found"),
            @ApiResponse(responseCode = "404", description = "Payment not found")
    })
    public ResponseEntity<InsurancePolicyPaymentResponseDTO> getPaymentById(
            @Parameter(description = "Payment unique identifier", required = true) @PathVariable Long paymentId) {
        InsurancePolicyPaymentResponseDTO payment = insurancePolicyPaymentService.getPaymentById(paymentId);
    ...
    @Operation(summary = "Update a policy payment",
            description = "Updates an existing policy payment record.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Payment successfully updated"),
            @ApiResponse(responseCode = "400", description = "Invalid input data or validation error"),
            @ApiResponse(responseCode = "404", description = "Payment not found")
    })
    public ResponseEntity<InsurancePolicyPaymentResponseDTO> updatePayment(
            @Parameter(description = "Payment unique identifier", required = true) @PathVariable Long paymentId,
            @Validated(OnUpdate.class) @RequestBody InsurancePolicyPaymentCreateDTO dto) {
        InsurancePolicyPaymentResponseDTO updated = insurancePolicyPaymentService.updatePayment(paymentId, dto);
    ...
    @Operation(summary = "Delete a policy payment",
            description = "Soft-deletes a payment record and reverts payment effects.")
    @ApiResponses(value = {
    ...
    public ResponseEntity<Void> deletePayment(
            @Parameter(description = "Payment unique identifier", required = true) @PathVariable Long paymentId) {
        insurancePolicyPaymentService.deletePayment(paymentId);
        return ResponseEntity.noContent().build();
    }
}
