...
public record FuelLoadDTO(
        ...
        @Schema(description = "Price per liter for this fuel type. Optional — if provided, overrides the gas station's listed price for the selected fuel type. " +
                ...
                example = "1200.50",
                nullable = true)
        @DecimalMin(value = "0.0001", message = "{fuelLoad.pricePerLiter.positive}", groups = {OnCreate.class, OnUpdate.class})
        @Digits(integer = 8, fraction = 4, message = "{validation.digits}", groups = {OnCreate.class, OnUpdate.class})
