...
public record GasStationDTO(
        ...
        @NotNull(message = "{gasStation.supplierId.required}", groups = OnCreate.class)
        ...
        @Schema(description = "List of fuel prices offered at this gas station. At least one price is required.",
                requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull(message = "{gasStation.prices.required}", groups = OnCreate.class)
        @Size(min = 1, message = "{gasStation.prices.required}", groups = {OnCreate.class, OnUpdate.class})
