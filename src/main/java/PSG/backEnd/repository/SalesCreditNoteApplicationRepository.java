...
@Repository
public interface SalesCreditNoteApplicationRepository extends JpaRepository<SalesCreditNoteApplication, Long> {

    List<SalesCreditNoteApplication> findByCreditNote_Id(Long creditNoteId);

    List<SalesCreditNoteApplication> findByInvoice_Id(Long invoiceId);

    /** Sum of credit applied to an invoice, excluding rows that belong to deleted credit notes. */
    @Query("SELECT COALESCE(SUM(a.amountApplied), 0) FROM SalesCreditNoteApplication a " +
