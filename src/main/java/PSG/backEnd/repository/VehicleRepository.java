...
public interface VehicleRepository extends JpaRepository<Vehicle, Long> {
    ...
    @Query("SELECT v FROM Vehicle v " +
            ...
            "WHERE (:licensePlate IS NULL OR LOWER(CAST(v.licensePlate AS string)) LIKE LOWER(CONCAT('%', :licensePlate, '%'))) " +
            "AND (:brand IS NULL OR LOWER(CAST(v.brand AS string)) LIKE LOWER(CONCAT('%', :brand, '%'))) " +
            "AND (:model IS NULL OR LOWER(CAST(v.model AS string)) LIKE LOWER(CONCAT('%', :model, '%'))) " +
            "AND (CAST(:year AS integer) IS NULL OR v.year = :year) " +
            "AND (:color IS NULL OR LOWER(CAST(v.color AS string)) LIKE LOWER(CONCAT('%', :color, '%'))) " +
            "AND (:nickName IS NULL OR LOWER(CAST(v.nickName AS string)) LIKE LOWER(CONCAT('%', :nickName, '%'))) " +
            "AND (:vehicleType IS NULL OR LOWER(CAST(vt.name AS string)) LIKE LOWER(CONCAT('%', :vehicleType, '%'))) " +
            "AND (:projectAreaName IS NULL OR LOWER(CAST(pa.name AS string)) LIKE LOWER(CONCAT('%', :projectAreaName, '%'))) " +
            "AND (:buildingName IS NULL OR LOWER(CAST(b.name AS string)) LIKE LOWER(CONCAT('%', :buildingName, '%'))) " +
            ...
            "AND (:search IS NULL OR (LOWER(CAST(v.licensePlate AS string)) LIKE LOWER(CONCAT('%', :search, '%')) " +
            "     OR LOWER(CAST(v.brand AS string)) LIKE LOWER(CONCAT('%', :search, '%')) " +
            "     OR LOWER(CAST(v.model AS string)) LIKE LOWER(CONCAT('%', :search, '%')) " +
            "     OR LOWER(CAST(v.nickName AS string)) LIKE LOWER(CONCAT('%', :search, '%'))))")
